...
spainFuelCodeMap()
{
    ...
    std::ifstream file("spain_lookup_table.csv");
    if (!file)
    {
        std::cerr << "Warning: unable to open spain_lookup_table.csv" << std::endl;
        loaded = true;
        return mapping;
    }
    ...
    while (std::getline(file, line))
    {
        ...
        if (token.empty())
            continue;
        int grid_value = std::stoi(token);

        // skip export value and description
        std::getline(ss, token, ',');
        std::getline(ss, token, ',');

        std::string fuel_code;
        std::getline(ss, fuel_code, ',');
        fuel_code = trim(fuel_code);
        if (!fuel_code.empty())
        {
            std::string normalized = toUpper(fuel_code);
            if (normalized.rfind("NON", 0) == 0 || normalized == "NF")
            {
                continue;
            }
            mapping[grid_value] = fuel_code;
        }
    }
    loaded = true;
    return mapping;
...
initialize_coeff(int scenario)
{
    ...
    if (scenario == 4)
    {
        ...
        // Fuel Model Parameters for Crown Fire driven by Scott & Burgan table
    const auto& lookup = spainFuelCodeMap();
    const auto& registry = FuelModelRegistry::instance();
    static std::unordered_set<std::string> missing_codes;
    if (lookup.empty())
    {
        std::cerr << "FuelModelSpain: spain_lookup_table.csv is empty; fm_parameters will not be populated\n"
                  << std::endl;
    }
    else
    {
        ...
        {
            const auto* record = registry.findByCode(pair.second);
            if (!record)
            {
                if (missing_codes.insert(pair.second).second)
                {
                    std::cerr << "FuelModelSpain: missing Scott & Burgan entry for code " << pair.second << std::endl;
                }
                continue;
            }
            std::vector<float> fp;
            fp.push_back(record->dead1 + record->dead10 + record->dead100 + record->live_herb + record->live_wood);
            fp.push_back(record->heat);
            fm_parameters[pair.first] = fp;
        }
    }
